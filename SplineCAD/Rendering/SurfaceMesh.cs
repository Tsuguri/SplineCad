...
namespace SplineCAD.Rendering
{
	public class SurfaceMesh : Mesh<PositionVertex>
	{
		...
		public SurfaceMesh(uint xDiv, uint yDiv, float from = 0.0f, float to = 1.0f)
		{
			...
			for (int i = 0; i < xDiv + 1; i++)
				...
				{
					vertices[i * (yDiv + 1) + j] = new PositionVertex((i / (float)xDiv)*(to-from)+from, (j / (float)yDiv) * (to - from) + from, 0);
				}
